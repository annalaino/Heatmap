...
def rain_fall():
    """
    Function sets a random value for rainfall in the WWTP catchment.
    """
    global RainInflow, RainLength
    current_time = round(gpsx.getValue("t"), 2)

    if current_time < 23:
        gpsx.setValue("rainfall91", RainInflow[1])
    elif current_time > RainLength:
        gpsx.setValue("rainfall91", RainInflow[0])
    else:
        gpsx.setValue("rainfall91", RainInflow[1])
    print("rain")
...
def start():
    try:
        global RainInflow,RainLength
        # choose random number between 0 and 2 with uniform distribution
        RainInflow[1] = random.uniform(0, 1.025)
        # set a random length of rain between 24 and 28
        RainLength = random.uniform(24,37) 
        print("start")
